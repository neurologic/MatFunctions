function fun = filtered_response_synaptic(spk_times, tau)
...
%
%   For examples on usage see motion_script.m in the github repository
%   MarvinT/EJ-lab-code
%
% Marvin Thielk 2013
% mthielk@salk.edu
...
% y = (exp(-(t-spiketime)/tau)).*((t-spiketime)>=0);

fun = @(t) nansum((exp((-(repmat(t, size(spk_times))-repmat(spk_times, size(t)))/tau)) .* ((repmat(t, size(spk_times))-repmat(spk_times, size(t)))>=0)),1);
